...
class EmployeeFilteredSelectMultiple(FilteredSelectMultiple):
    def __init__(self, verbose_name, is_stacked, attrs=None, choices=(), aln_labels: list = list):
        super().__init__(verbose_name, is_stacked, attrs, choices)
        self.aln_labels = aln_labels

    def create_option(self, name, value, label, selected, index, subindex=None, attrs=None):
        option_dict = super(EmployeeFilteredSelectMultiple, self).create_option(name, value, label, selected, index,
                                                                                subindex=None, attrs=None)

        value = ModelChoiceIteratorValue(value, instance=name)
        employee = Employee.objects.get(pk=value.__str__())
        g_label = generate_label(employee, self.aln_labels)
        option_dict['label'] = f'{label}' if not g_label else g_label
        return option_dict
